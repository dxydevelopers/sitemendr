...
export default function AdminBuildPipeline({ dashboard }: AdminBuildPipelineProps) {
  ...
  }, [selectedProjectRequest?.id, selectedProjectRequest?.status, selectedProjectRequest?.completionAcknowledgedAt, handoffChatLoadedRequestId]);
          ...
          {selectedAdminBuildChapter.id === 'launch' && (
            <div className="border-y border-expert-green/25 py-5">
              ...
              <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_15rem]">
                ...
                <div className="border-t border-white/10 pt-5 xl:border-l xl:border-t-0 xl:pl-5 xl:pt-0">
                  {selectedProjectRequest.status === 'handoff' && !selectedProjectRequest.completionAcknowledgedAt && (
                    <div className="flex h-full flex-col">
                      <div className="flex items-center justify-between gap-4">
                        <p className="text-[9px] font-black uppercase tracking-[0.16em] text-white/34">Handoff discussion</p>
                        <span className="text-[9px] font-black uppercase tracking-[0.12em] text-white/30">{handoffChatLoading ? 'Loading' : `${handoffChatMessages.length} msg${handoffChatMessages.length === 1 ? '' : 's'}`}</span>
                      </div>
                      <div className="mt-3 max-h-56 flex-1 space-y-3 overflow-y-auto border-y border-white/10 py-3">
                      ...
                      <textarea value={handoffChatDraft} onChange={(e) => setHandoffChatDraft(e.target.value)} rows={2} className="mt-3 w-full resize-none border-0 border-b border-white/10 bg-transparent px-0 py-2 text-sm text-white outline-none transition placeholder:text-white/24 focus:border-ai-blue/60" placeholder="e.g. Are you happy with the handoff?" />
                  ...
                  {selectedProjectRequest.status === 'handoff' && selectedProjectRequest.completionAcknowledgedAt && (
                    <div>
                      <div className="flex items-center gap-2 text-expert-green"><Check className="h-4 w-4" /><p className="text-[9px] font-black uppercase tracking-[0.16em]">Client accepted handoff</p></div>
                      <p className="mt-3 text-[9px] font-black uppercase tracking-[0.16em] text-white/34">Closeout</p>
                      <div className="mt-3 space-y-3 border-y border-white/10 py-4">
                        ...
                        <div className="flex items-center justify-between gap-4 pt-1">
                          ...
                          <span className={`text-[10px] font-black uppercase tracking-[0.12em] ${selectedAgreementBalance <= 0 || selectedProjectRequest.finalPaymentConfirmedAt ? 'text-expert-green' : 'text-amber-300'}`}>{selectedAgreementBalance <= 0 ? 'Not owed' : selectedProjectRequest.finalPaymentConfirmedAt ? 'Received' : 'Waiting'}</span>
                        </div>
                      </div>
                      {selectedAgreementBalance > 0 && !selectedProjectRequest.finalPaymentConfirmedAt && (
                        <button type="button" onClick={() => handleMarkFinalPaymentReceived(selectedProjectRequest.id)} disabled={submitting} className="mt-4 flex min-h-10 w-full items-center justify-between gap-3 border border-amber-300/25 px-3 py-2 text-left text-[10px] font-black uppercase tracking-[0.14em] text-amber-300 transition hover:bg-amber-300/10 hover:text-white disabled:opacity-45">Mark final payment received<ChevronRight className="h-4 w-4" /></button>
                      )}
                      <textarea defaultValue={selectedProjectRequest.completionNotes || ''} onBlur={(e) => handleUpdateProjectRequest(selectedProjectRequest.id, { completionNotes: e.target.value })} className="mt-4 h-16 w-full resize-none border-0 border-b border-white/10 bg-transparent px-0 py-2 text-sm leading-6 text-white outline-none transition placeholder:text-white/24 focus:border-expert-green/60" placeholder="Final completion summary..." />
                      <button type="button" onClick={() => handleUpdateProjectRequest(selectedProjectRequest.id, { status: 'completed' })} disabled={submitting || (selectedAgreementBalance > 0 && !selectedProjectRequest.finalPaymentConfirmedAt)} className="mt-4 flex min-h-11 w-full items-center justify-between gap-3 border border-expert-green/25 px-3 py-2 text-left text-[10px] font-black uppercase tracking-[0.14em] text-expert-green transition hover:bg-expert-green/10 hover:text-white disabled:cursor-not-allowed disabled:opacity-40">Close build<ChevronRight className="h-4 w-4" /></button>
                    </div>
                  )}

                  {selectedProjectRequest.status === 'completed' && (
                    <div>
                      <div className="flex items-center gap-2 text-expert-green"><Check className="h-4 w-4" /><p className="text-[9px] font-black uppercase tracking-[0.16em]">Build closed</p></div>
