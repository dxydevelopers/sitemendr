            ...
            <div className="border-y border-expert-green/25 py-5">
              {isProjectRequest(currentBuildRecord) && ['launched', 'handoff', 'completed'].includes(currentBuildStatus) ? (
                <>
                  ...
                  {/* Handoff: not yet accepted -> default summary, or the handoff chat once an issue has been raised */}
                  {currentBuildStatus === 'handoff' && !currentBuildRecord.completionAcknowledgedAt && (
                    !handoffChatStarted ? (
                      <>
                        ...
                        <div className="mt-4 grid border-y border-white/10 sm:grid-cols-2 sm:divide-x sm:divide-white/10">
                          ...
                          <button type="button" onClick={() => handleOpenHandoffChat(currentBuildRecord)} className="flex items-center justify-between gap-3 border-t border-white/10 px-4 py-4 text-left text-[10px] font-black uppercase tracking-[0.16em] text-ai-blue transition hover:bg-white/[0.04] hover:text-white sm:border-t-0">
                            <span className="flex items-center gap-3"><MessageSquare className="h-4 w-4" /> Report handoff issue</span><ChevronRight className="h-4 w-4" />
                          </button>
                        </div>
                      </>
                    ) : (
                      <div className="mt-6 border-y border-white/10">
                        <div className="flex items-center justify-between gap-4 border-b border-white/10 py-4">
                          <div>
                            ...
                            <p className="mt-1 text-xs leading-5 text-white/44">Tell the team what's missing. You can accept handoff here once it's sorted.</p>
                          </div>
                          <span className="text-[10px] font-black uppercase tracking-[0.14em] text-white/34">{handoffChatLoading ? 'Loading' : `${handoffChatMessages.length} message${handoffChatMessages.length === 1 ? '' : 's'}`}</span>
                        </div>
                        <div className="max-h-72 space-y-3 overflow-y-auto py-4">
                        ...
                        <div className="border-t border-white/10 py-4">
                          <textarea value={handoffChatDraft} onChange={(e) => setHandoffChatDraft(e.target.value)} rows={2} className="w-full resize-none border-0 border-b border-white/10 bg-transparent px-0 py-3 text-sm leading-6 text-white outline-none transition placeholder:text-white/24 focus:border-ai-blue/60" placeholder="Describe the handoff issue..." />
                          <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                            ...
                            {handoffChatMessages.some(m => m.senderRole === 'admin') && (
                              <button type="button" onClick={() => handleHandoffResponse(currentBuildRecord, 'complete')} disabled={handoffSubmitting} className="inline-flex min-h-10 items-center gap-2 border border-expert-green/30 px-3 py-2 text-[10px] font-black uppercase tracking-[0.14em] text-expert-green transition hover:bg-expert-green/10 hover:text-white disabled:opacity-40">
                                {handoffSubmittingAction === 'complete' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Check className="h-3.5 w-3.5" />} Accept handoff - mark as complete
                              </button>
                            )}
                          </div>
                        </div>
                  ...
                  {currentBuildStatus === 'handoff' && currentBuildRecord.completionAcknowledgedAt && (
                    <div className="mt-6 border-y border-expert-green/25 py-6">
                      <div className="flex flex-wrap items-center gap-3"><Check className="h-4 w-4 text-expert-green" /><p className="text-[9px] font-black uppercase tracking-[0.18em] text-expert-green">Handoff accepted</p></div>
                      ...
                      {agreementBalanceAmount > 0 ? (
                        currentBuildRecord.finalPaymentConfirmedAt ? (
                          <div className="mt-5 flex items-center gap-3 border border-expert-green/25 px-3 py-3 text-[10px] font-black uppercase tracking-[0.14em] text-expert-green"><Check className="h-4 w-4" />Final payment received - finalizing your project</div>
                        ) : (
                          <button type="button" onClick={() => handleFinalBalancePayment(currentBuildRecord, activeAgreementPaymentMethod, agreementBalanceAmount)} disabled={finalPaymentSubmitting} className="mt-5 flex min-h-12 w-full items-center justify-between gap-3 border border-expert-green/35 px-4 py-3 text-left text-[10px] font-black uppercase tracking-[0.16em] text-expert-green transition hover:border-expert-green hover:bg-expert-green/10 hover:text-white disabled:opacity-50 sm:max-w-sm">
                            ...
                            {finalPaymentSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <ChevronRight className="h-4 w-4" />}
                          </button>
                        )
                      ) : (
                        <p className="mt-5 text-sm font-semibold leading-6 text-white/58">No balance remains. The team will mark this project complete shortly.</p>
                      )}
                    </div>
                  )}
                  ...
                  {currentBuildStatus === 'completed' && (
                    <div className="mt-6 border-y border-expert-green/25 py-8 text-center">
                      <div className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-expert-green/10 text-expert-green"><Check className="h-6 w-6" /></div>
                      <h4 className="mt-4 text-2xl font-black tracking-tight text-white sm:text-3xl">Project complete</h4>
                      <p className="mx-auto mt-2 max-w-md text-sm font-semibold leading-7 text-white/60">{currentBuildRecord.completionNotes || 'This build is fully paid, handed off, and complete.'}</p>
                      <div className="mx-auto mt-6 grid max-w-sm gap-3 border-y border-white/10 py-4 sm:grid-cols-2">
                      ...
                      {currentBuildRecord.launchUrl && (
                        <a href={currentBuildRecord.launchUrl} target="_blank" rel="noreferrer" className="mt-6 inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.14em] text-ai-blue transition hover:text-white">Open live site <ExternalLink className="h-3.5 w-3.5" /></a>
                      )}
                    </div>
                  )}
                </>
