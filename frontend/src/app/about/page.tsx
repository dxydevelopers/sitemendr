      ...
      <div className="relative z-10 mx-auto max-w-7xl px-5 sm:px-6 md:px-10">
        <section className={`mb-24 text-center transition duration-700 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
          <div className="inline-flex items-center gap-3 border-b border-ai-blue/40 px-1 pb-3">
            ...
            <span className="text-xs font-bold uppercase tracking-[0.1em] text-ai-blue">
              Sitemendr company system
            </span>
          </div>

          <h1 className="mx-auto mt-8 max-w-5xl text-2xl font-bold leading-[0.96] tracking-tight sm:text-3xl md:text-4xl lg:text-5xl">
            Digital work,
            <br />
            <span className="text-white/48">made traceable.</span>
          </h1>

          <p className="mx-auto mt-8 max-w-3xl text-base leading-8 text-white/64 md:text-xl">
          ...
          <div className="mt-12 flex flex-wrap justify-center gap-6 md:gap-10">
            {stats.map((stat) => (
              <div key={stat.label} className="min-w-[190px] border-t border-white/10 pt-5 text-center">
                ...
                <p className="mt-2 text-xs font-bold uppercase tracking-[0.1em] text-white/48">{stat.label}</p>
                <p className="mt-2 text-xs leading-5 text-white/48">{stat.note}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-24 border-y border-white/10 py-12 md:py-16">
          <div className="mb-10 flex flex-wrap justify-center gap-2">
            {tabs.map((tab) => (
              <button
                ...
                className={`inline-flex min-h-[46px] items-center gap-3 px-5 text-xs font-bold uppercase tracking-[0.1em] transition ${
                  activeTab === tab.id ? 'bg-white text-black' : 'text-white/48 ring-1 ring-white/10 hover:text-white'
                }`}
              >
                {tab.icon}
                {tab.label}
          ...
          <div className="mx-auto grid max-w-5xl gap-10 lg:grid-cols-[0.82fr_1.18fr] lg:items-start">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.1em] text-ai-blue">Company position</p>
              <h2 className="mt-5 text-xl font-bold tracking-tight md:text-2xl">{tabContent[activeTab].title}</h2>
            </div>
            <div>
              ...
              <div className="mt-8 grid gap-4 md:grid-cols-2">
                {tabContent[activeTab].points.map((point) => (
                  <div key={point} className="flex items-start gap-3">
                    ...
                    <span className="text-sm leading-7 text-white/64">{point}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="mb-24 grid gap-12 lg:grid-cols-2 lg:items-center">
            ...
            <div className="relative aspect-[4/3] overflow-hidden">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 md:p-8">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xs font-bold uppercase tracking-[0.1em] text-ai-blue">Operating idea</p>
                    <h3 className="mt-1 text-2xl font-bold tracking-tight">Workspace-first delivery</h3>
                  </div>
                </div>
                <div className="mt-5 flex flex-wrap gap-5 text-xs font-semibold uppercase tracking-[0.1em] text-white/64">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.1em] text-tech-purple">Our thesis</p>
            <h2 className="mt-5 text-xl font-bold leading-tight tracking-tight md:text-3xl">
          ...
          <div className="mt-12 grid gap-x-10 gap-y-12 md:grid-cols-2 lg:grid-cols-3">
            {features.map((feature) => (
              <div key={feature.title} className="group border-t border-white/10 pt-7">
                ...
                <h3 className="mt-8 text-xl font-bold tracking-tight text-white">{feature.title}</h3>
                <p className="mt-4 text-sm leading-7 text-white/64">{feature.desc}</p>
                <ArrowUpRight className="mt-8 h-5 w-5 text-white/24 transition group-hover:text-white/64" />
              </div>
            ))}
          </div>
        </section>
                  ...
                  <div className={`${index % 2 === 0 ? 'md:pr-12 md:text-right' : 'md:pl-12'}`}>
                    <div className="border-t border-white/10 pt-6">
                      <div className={`mb-5 inline-flex items-center gap-3 ${index % 2 === 0 ? 'md:ml-auto' : ''}`}>
                        ...
                        <span className="text-xs font-bold uppercase tracking-[0.1em] text-white/64">{item.year}</span>
                      </div>
            ...
            {principles.map((principle) => (
              <div key={principle.title} className="border-t border-white/10 pt-7">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-4xl font-bold tracking-tight text-white/24">{principle.stat}</span>
                </div>
                <h3 className="mt-8 text-xl font-bold tracking-tight">{principle.title}</h3>
                <p className="mt-4 text-sm leading-7 text-white/64">{principle.desc}</p>
              </div>
            ))}
          </div>
        </section>
            ...
            <div className="mt-10 grid gap-x-8 gap-y-10 sm:grid-cols-2">
              {team.map((member) => (
                <div key={member.name} className="border-l border-white/10 pl-6">
                  ...
                  <div className="mt-8 text-3xl font-bold tracking-tight">{member.count}</div>
                  <h3 className="mt-2 text-lg font-bold">{member.name}</h3>
                  <p className="mt-1 text-xs font-bold uppercase tracking-[0.1em] text-white/36">{member.role}</p>
                  <p className="mt-4 text-sm leading-7 text-white/48">{member.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-12 grid grid-cols-2 gap-x-8 gap-y-6 md:grid-cols-3 lg:grid-cols-4">
            {technologies.map((tech) => (
              <div key={tech.name} className="border-b border-white/10 pb-5 transition hover:border-ai-blue/40">
                ...
                <p className="mt-5 text-sm font-bold text-white">{tech.name}</p>
                <p className="mt-2 text-xs font-bold uppercase tracking-[0.1em] text-white/36">{tech.category}</p>
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="mt-12 grid gap-x-10 gap-y-12 md:grid-cols-3">
            {testimonials.map((testimonial) => (
              <div key={testimonial.name} className="relative border-t border-white/10 pt-7">
                <Quote className="absolute right-0 top-7 h-8 w-8 text-white/24" />
                <div className="mb-6 flex gap-1">
                  {[...Array(testimonial.rating)].map((_, index) => (
                    <Star key={index} className="h-4 w-4 fill-amber-300 text-amber-300" />
                  ))}
                </div>
                <p className="text-sm leading-7 text-white/64">&quot;{testimonial.content}&quot;</p>
                <div className="mt-7 flex items-center gap-4">
                  <div className="grid h-12 w-12 place-items-center bg-white text-sm font-bold text-black">{testimonial.avatar}</div>
                  <div>
                    <p className="text-sm font-bold">{testimonial.name}</p>
                    <p className="mt-1 text-xs font-bold uppercase tracking-[0.1em] text-white/36">{testimonial.role}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <section className="mb-16 overflow-hidden border-y border-white/10">
          <div className="grid gap-0 lg:grid-cols-[1fr_0.82fr]">
            <div className="p-8 md:p-12 lg:p-16">
              ...
              <h2 className="mt-8 max-w-3xl text-xl font-bold leading-tight tracking-tight md:text-3xl">
...
function SectionIntro({
  ...
  return (
    <div className={align === 'center' ? 'mx-auto max-w-3xl text-center' : 'max-w-3xl'}>
      {eyebrow ? <p className="text-xs font-bold uppercase tracking-[0.1em] text-ai-blue">{eyebrow}</p> : null}
      <h2 className={eyebrow ? 'mt-5 text-xl font-bold leading-tight tracking-tight md:text-2xl' : 'text-xl font-bold leading-tight tracking-tight md:text-2xl'}>{title}</h2>
